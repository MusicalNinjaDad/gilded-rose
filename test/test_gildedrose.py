...
def test_qualitymax50(items):
    gr = GildedRose(items=items)
    gr.update_quality()
    assert gr.items[0].quality <= MAX_QUALITY


def test_conjured():
    gr = GildedRose(items=[Item(name="Conjured Mana Cake", sell_in=3, quality=6)])
    gr.update_quality()
    assert gr == GildedRose(items=[Item(name="Conjured Mana Cake", sell_in=2, quality=4)])
...
def test_sulfurasalways80(items):
    gr = GildedRose(items=items)
    gr.update_quality()
    assert gr.items[0].quality == SULFURAS_QUALITY
